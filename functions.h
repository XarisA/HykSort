//
// Created by xaris on 22/01/2021.
//

#include <cstring>

#ifndef HYKSORTINCPP_FUNCTIONS_H
#define HYKSORTINCPP_FUNCTIONS_H

#endif //HYKSORTINCPP_FUNCTIONS_H
...
std::vector<unsigned int> init_array(int size,int task)
{
    /* Intializes random number generator */
    srand((unsigned) time(NULL)+task*100);

    std::vector<unsigned int> v(size); // contains size number of 0's
    std::generate (v.begin(), v.end(), []{ return rand() % 100; });
    return v;
}
void print_array(std::vector<unsigned int> v, int size)
{
    //This program prints the n values of an array
    int i;
    //
    printf("[");
        for(int i=0; i<size-1; i++){printf("%2d, ",v[i]);}
    printf("%2d]",v[size-1]);
}
void print_array_in_process(std::vector<unsigned int> v, int n, int p, int rank )
{
    printf("Array in process %d of %d with %d elements : ",rank,p,n);
    //This program prints the n values of an array
    int i;
    //
    printf("[");
    for(int i=0; i<n-1; i++){printf("%2d, ",v[i]);}
    printf("%2d]",v[n-1]);
    printf ( "\n\n");
}

void scan(int *A, int* B, int cnt){
    int p=omp_get_max_threads();
    if(cnt<100*p){
        for(int i=1;i<cnt;i++)
            B[i]=B[i-1]+A[i-1];
...
void MergeLists( std::vector<unsigned int> &listA, std::vector<unsigned int> &listB) {
    ...
    std::vector<unsigned int> scratch_list(list_size);
